...
// 返り値はrectのDOMを返す
function draw_h_stroke(arr, n_th) {
  var rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
  var glyph_origin = calc_glyph_origin(n_th);
...
// data.jsにあるhorizontalの中の配列データarr, 何文字目かの情報n_th
// 返り値はrectのDOMを返す
function draw_v_stroke(arr, n_th) {
  var rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
  var glyph_origin = calc_glyph_origin(n_th);
...
function draw_sl_stroke(arr, n_th) {
...
function draw_sl_stroke_line(arr, n_th) {
  var glyph_origin = calc_glyph_origin(n_th);
  var line = document.createElementNS("http://www.w3.org/2000/svg", "line");
  var stroke_path = [glyph_origin.x + arr[0] * config.width / config.step,  // 始点x座標
                     glyph_origin.y + calc_height(arr[1]),  // 始点y座標
                     glyph_origin.x + arr[2] * config.width / config.step,  // 終点x座標
                     glyph_origin.y + calc_height(arr[3])   // 始点y座標
  ];
...
function create_glyphs(text) {
  text_arr = text.split("");
  var n = text_arr.length;
  
  // 文字を入れるsvg要素を作成
  var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  // svgの外枠を設定する
  svg.setAttribute("width", n * (config.width + config.v_stroke_width) + n * config.kerning);
  svg.setAttribute("height", config.width + config.ascender + config.descender + 2 * config.h_stroke_width);

  // グリフのエレメントを入れていく
  for(var i = 0; i < text_arr.length; i++) {
    var glyph = glyphs[text_arr[i]];

    if(glyph === undefined) {
      msg = ((i+1) + "文字目の" + text_arr[i] + "は存在しないグリフです");
      continue;
    }

    console.log(text_arr[i] + " : " + glyph.h.length + " / " + glyph.v.length + " / " + glyph.sl.length)

    // 水平な画
    if(!(glyph.h.length === 0)) {
      var h_rect = [];
      for(var j = 0; j < glyph.h.length; ++j) {
        h_rect[j] = draw_h_stroke(glyph.h[j], i + 1);
    ...
    if(!(glyph.v.length === 0)) {
      ...
      for(var j = 0; j < glyph.v.length; ++j) {
        v_rect[j] = draw_v_stroke(glyph.v[j], i + 1);
        svg.appendChild(v_rect[j]);
      }
    }
    // 斜めの画
    console.log(!(glyph.sl.length === 0))
    if(!(glyph.sl.length === 0)) {
      var sl_polygon = [];
      for(var j = 0; j < glyph.sl.length; ++j) {
        sl_polygon[j] = draw_sl_stroke(glyph.sl[j], i + 1);
